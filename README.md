...
The soft top edge is five contours of one field rather than a painted gradient. Where they spread apart the model is unsure where the layer ends, and the edge is wide because the reading is. It also holds the picture still: two points of cloud cover move a single contour by up to a fifth of the chart and the stack by under a twelfth, because the eye reads the stack.

Five, not more. Past a point the extra bands say "foggy" a second time and the picture starts reading as a topographic map. Three is too few to hold still. The contours are a sampling of a continuous field rather than strata in it, so they are drawn with a falling alpha and softened, and the field runs off the top of the frame rather than stopping at a hard edge.
