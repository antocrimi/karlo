...
**The vertical profile is data, not styling.** A marine layer has a base. When it sits aloft there is clear air underneath, which is exactly why you can see across the city beneath a low ceiling; when it is on the deck there is not. The app has always said which of those is happening, in words, in `note()`, and never showed it.

The body is now painted twice over one path. `fogAir` is densest at the layer's top and thins toward the ground. `fogDeck` runs the other way and is gated on `here`, cover at each place's own altitude, so it only appears where the fog genuinely reaches street level and the mass stays solid to the pavement on those hours. Both vertical gradients are anchored to the body's own top each frame rather than to the frame, or a shallow layer would fade across a depth it does not have.

The horizontal ramps ride in **masks** rather than in fills, because a mask's luminance multiplies the fill's alpha, and that is what lets the body carry a horizontal density and a vertical profile at once. Peak opacity went from `.22` to `.36` at the same time: the ridge hairline had already made the terrain readable against a denser sky, so the headroom was there and unused.
