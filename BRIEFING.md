...
**The geometry is single-sourced, and now it is single-sourced locally.** Each spot has its own `sun`, the height at which `O` falls through 45%. A spot is in daylight when `elev >= sun`, and because `O` is monotone that is the *same statement* as `overhead < CLEAR` — one rule asked twice, not two rules kept in step. `test.js` asserts the equivalence directly. The band is five contours of the same `O`, so the CLEAR contour at a spot's own x is the line its dot is judged against. The picture cannot contradict the list at any x, which is what the previous two attempts could not guarantee. Visibility remains a separate gate. Mid-level cloud no longer needs one: `O` carries it.

**The soft edge is measured now, not painted.** The band was a single path with a fixed `0 / .24 / .50` gradient, which performed uncertainty without measuring any. It is five contours of `O` bracketing CLEAR, each a real iso-line. Where the contours spread the model is unsure of the top and the edge is wide; where they crowd it is tight. The ensemble is also what makes it hold still: perturbing one pressure level by two points moves the single CLEAR contour up to 42 px of chart and the stack under 20. `test.js` asserts the stack is at least twice as steady as any one contour.

**Five, and the count is load-bearing in both directions.** It started at nine, which carried no more meaning and read as a topographic map. Three drops the ensemble to 29 px of movement and fails the stability bar. Four passes only by dropping CLEAR out of the ladder, and CLEAR has to be drawn because it is the line every dot is judged against. Five is the fewest that keeps both properties.

**The contours are a sampling of a continuous field, not strata in it, and the rendering now says so.** Three changes on 30 August, all visual, none touching path data:
- **Alphas fall off** (`.20 .16 .12 .07 .03`) instead of being uniform. Equal steps put an equal-sized edge at the top of the stack, which is the one place it must not be seen. Multiplied out they land on `.47`, near the `.50` the old gradient bottomed out at.
- **A small blur** (`stdDeviation 5`) returns the sampling to the field it came from. Measured at 0.8 ms per scrub frame under a 6× CPU throttle, against 36.7 ms for the frame as a whole, so the cost is noise. It changes no `d` attribute, so the geometry the verdict is checked against is untouched.
- **The field runs off the top and fades into the page.** `yFog` no longer clamps to `y=2`. Clamping stacked every contour on one line and gave a socked-in hour a flat grey lid, so the whole picture read as a grey rectangle with hills in it. Note `yFog` derives from `Y0` and `Y_PER_M` directly rather than calling `yM`, because `yM`'s own clamp would reimpose the lid.
